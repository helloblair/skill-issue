...
Maintain a learning journal that explains *what happened and why* in plain language. This is for a developer who is learning, so entries should walk through the process step by step so they can re-read it later and fully understand what occurred. These entries double as interview prep, study notes, and debugging reference.
...
3. **Get today's date.** Run `date +%Y-%m-%d` in the shell to get the current date. Always use this command. Don't hardcode or guess the date.
